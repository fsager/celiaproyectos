...
select a.id assignment_id,ue.userid userid,u.email,c.fullname course_fullname,c.shortname course_shortname,c.category course_category
  ...
  join celiacie_moodle2.mdl_user_enrolments ue on e.id=ue.enrolid
  join celiacie_moodle2.mdl_user u on ue.userid=u.id
where not exists (select 1 from seguimiento_alumnos.cel_alerta_enviada where ale_obj_tipo='mdl_assignment' and ale_obj_id=a.id and ale_alerta='ALU_NUEVO_TP' and usr_id=ue.userid)
;
...
select a.id assignment_id,ue.userid userid,u.email,c.fullname course_fullname,c.shortname course_shortname,c.category course_category
  ...
  and not exists (select 1 from seguimiento_alumnos.cel_alerta_enviada where ale_obj_tipo='mdl_assignment' and ale_obj_id=a.id and ale_alerta='ALU_TP_VENCIDO' and usr_id=ue.userid)
;
